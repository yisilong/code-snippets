...
$api_sina = 'https://api.weibo.com/2/short_url/expand.json?source=2006100199'; // get方式
$api_baidu = 'http://dwz.cn/query.php'; // post方式
$query_str = $_SERVER['QUERY_STRING'];
// 分解出url  dest=baidu&url=http://t.cn/h5mwx&url=http://t.cn/h5gEb
$urls = array();
...
if ($dest == 'sina'){
	...
	foreach($urls as $url){
		$ncount++;
		$str_request .= '&url_short='.urlencode($url);
		if ($ncount == 20){
	...
	foreach($urls as $url){
		curl_setopt($ch,CURLOPT_POSTFIELDS,'tinyurl='.urlencode($url));
		$strRes=curl_exec($ch);
		$arrResponse=json_decode($strRes,true);
		if($arrResponse['status'] == 0)
		{
			$tinyurls[$arrResponse['longurl']] = $url;
		}
	}
	curl_close($ch);
}
