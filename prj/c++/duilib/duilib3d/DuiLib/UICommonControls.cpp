...
{
	::ZeroMemory(&m_rcForePadding, sizeof(m_rcForePadding));
...
void CProgressUI::SetForePadding(RECT rc)
{
	m_rcForePadding=rc;
	Invalidate();
}
RECT CProgressUI::GetForePadding()
{
	return m_rcForePadding;
}
void CProgressUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("forepadding")) == 0 ) 
	{
		RECT rcBorderRect = { 0 };
		LPTSTR pstr = NULL;
		rcBorderRect.left = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr); 
		rcBorderRect.top = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr);    
		rcBorderRect.right = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
		rcBorderRect.bottom = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);    
		SetForePadding(rcBorderRect);
	}
    else CLabelUI::SetAttribute(pstrName, pstrValue);
}
...
void CProgressUI::PaintStatusImage(HDC hDC)
{
    ...
    if( m_bHorizontal ) 
	{
		float fPercent = (float)(m_nValue - m_nMin) / (m_nMax - m_nMin);
        rc.right = (LONG)(fPercent * (m_rcItem.right - m_rcItem.left - m_rcForePadding.left - m_rcForePadding.right) + 0.5 + m_rcForePadding.left);
        rc.bottom = m_rcItem.bottom - m_rcItem.top;
    }
    else 
	{
		float fPercent = (float)(m_nMax - m_nValue) / (m_nMax - m_nMin);
        rc.top =  (LONG)(fPercent * (m_rcItem.bottom - m_rcItem.top - m_rcForePadding.top - m_rcForePadding.bottom) + 0.5 + m_rcForePadding.bottom);
    ...
    if( !m_sForeImage.IsEmpty() ) 
	{
        ...
        m_sForeImageModify.SmallFormat(_T("source='%d,%d,%d,%d' dest='%d,%d,%d,%d'"), rc.left, rc.top, rc.right, rc.bottom, rc.left, rc.top, rc.right, rc.bottom);
