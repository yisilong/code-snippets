...
CStdString::CStdString(LPCSTR lpsz, int nLen) : m_pstr(m_szBuffer)
{      
	ASSERT(!::IsBadStringPtrA(lpsz,-1));
	m_szBuffer[0] = '\0';
	int cchStr = (int) strlen(lpsz) + 1;
	LPWSTR pwstr = NULL;
	pwstr = (LPWSTR) _alloca(cchStr*2);
	if( pwstr != NULL )
	{
		::MultiByteToWideChar(CP_ACP, 0, lpsz, -1, pwstr, cchStr) ;
...
const CStdString& CStdString::operator=(LPCSTR lpStr)
{
    ...
    if( pwstr != NULL ) 
	{
		::MultiByteToWideChar(CP_ACP, 0, lpStr, -1, pwstr, cchStr) ;
	}
    Assign(pwstr);
    return *this;
}
