    ...
    case WM_PAINT:
        {
			...
			else if( !m_animation.IsJobsEmpty() ) 
			{
				...
				if( !m_animation.PrepareAnimation() ) m_animation.RemoveJobs();
    ...
    case WM_SIZE:
        {
			...
			// yisilong add
			//当对话框尺寸变化时 删除动画 job
			if( m_animation.IsAnimating() ) m_animation.RemoveJobs();
    ...
    case WM_LBUTTONDOWN:
        {
			...
			m_animation.RemoveJobs(); //yisilong add
        }
        break;
...
bool CPaintManagerUI::AddAnimJob(const CAnimJobUI& job)
{
	...
	m_animation.Init(m_hWndPaint);
	return m_animation.AddJob(pJob);
	//return 1;
}
// end yisilong add
