...
bool CCloseAnimation::Create(HWND hWndDest,UINT uDuration,bool bCloseDest,int iCmdShow/* = SW_HIDE*/,AnimStyle eStyle /*= STYLE_CLOSE_NORMAL*/)
{
	...
	if (IsIconic(hWndDest) != 0) //目标窗口最小化
	{
		...
		if (bCloseDest)
		{
			PostMessage(hWndDest,WM_SYSCOMMAND,SC_CLOSE,0);
		}
		return true;
	}
	...
	{
		if (!GetShellIconPos(m_rcShellIcon,hWndDest))
		{
			return false;
		}
	}
...
{
	long cxShellIcon = m_rcShellIcon.right - m_rcShellIcon.left; // 系统托盘的宽度
	long cyShellIcon = m_rcShellIcon.bottom - m_rcShellIcon.top; // 系统托盘的高度
	int nStepLeft = (int)((m_rcShellIcon.left - m_rcDest.left)/(float)m_uStepCount);  // 递增的左坐标
	int nStepTop = (int)((m_rcShellIcon.top - m_rcDest.top)/(float)m_uStepCount);	  // 递增的上坐标
	int fStepInflateW = (int)(((m_rcDest.right-m_rcDest.left - cxShellIcon))/(float)m_uStepCount+0.1f); // 递减的宽度
	int fStepInflateH = (int)(((m_rcDest.bottom-m_rcDest.top - cyShellIcon))/(float)m_uStepCount+0.1f); // 递减的高度
	
	long cx = rcWindow.right - rcWindow.left; //上一次宽度
	long cy = rcWindow.bottom - rcWindow.top; //上一次高度

	if (rcWindow.left < m_rcShellIcon.left)
	{
		rcWindow.left += nStepLeft;
		if(rcWindow.left > m_rcShellIcon.left)
		{
			rcWindow.left = m_rcShellIcon.left;
		}
		rcWindow.top += nStepTop;
		if (rcWindow.top > m_rcShellIcon.top)
		{
			rcWindow.top = m_rcShellIcon.top;
		}
...
UINT CCloseAnimation::MyThread(LPVOID lpParam)
{
	CCloseAnimation* pThis = static_cast<CCloseAnimation*>(lpParam);
	return pThis->ThreadRun();
}
UINT CCloseAnimation::ThreadRun()
{
	RECT rcDest;
	...
	m_hWnd = CreateWindowEx(WS_EX_TOOLWINDOW,_T("closeAnimation"),NULL,WS_POPUP,rcDest.left,rcDest.top,rcDest.right-rcDest.left,rcDest.bottom-rcDest.top,NULL,NULL,(HINSTANCE)GetWindowLong(m_hWndDest,GWL_HINSTANCE),this);
	if (m_hWnd == NULL)
	{
		return false;
	}
...
LRESULT CCloseAnimation::OnCreate( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	if (!m_bCloseDest)
	{
		ShowWindow(m_hWndDest,m_iCmdShow);
	}
	else
	{
