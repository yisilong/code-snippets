//Missing getElementById,add this function to document
...
//make a function instead of getElementById
function $()
{
...
window.ADS.addEventListener = addEventListener;
...
window.ADS.removeEventListener = removeEventListener;
//use className get elements
...
window.ADS.prepenChild = prepenChild;
...
function removeChildren(parent)
{
	...
	return parent; //Return the parent again so you can stack the methods
}
window.ADS.removeChildren = removeChildren;
//Put the given object in teh context of the given method.
function bindFuntion(obj,func)
{
	return function(){
		func.apply(obj,arguments);
	};
}
window.ADS.bindFuntion = bindFuntion;
//Retrieve the size of browser window
function getBrowserWindowSize()
{
    // window.innerWidth and window.innerHeight for most browsers
    // document.documentElement.clientWidth and document.documentElement.clientHeight for MSIE in strict mode
    // document.body.clientWidth and document.body.clientHeight for MSIE in quirks mode
	return {
		'width': window.innerWidth || (document.documentElement && document.documentElement.clientWidth) || document.body.clientWidth;
		'heith': window.innerHeight || (document.documentElement && document.documentElement.clientHeight) || document.body.clientHeight;
	};
}
...
window.ADS.getScroll = getScroll;
...
window.walkElementsLinear = walkElementsLinear;
...
function walkTheDOMWithAttributes(node,func,depth,returnedFromParent)
{
    ...
    if(root.nodeType != ADS.nodeType.ATTRIBUTE_NODE) {
        node = root.firstChild;
        while(node) {
...
//Convert hyphenated hello-world strings to camel case helloWorld strings.
function camelize(s)
{
...
//Convert camel case helloWorld strings to hyphenated hello-world strings.
...
function addLoadEvent(loadEvent,waitForImages) {
    ...
    if(waitForImages) {
        return addEventListener(window, 'load', loadEvent);
    }
...
function stopPropagation(eventObject)
{
	...
	else if(eventObject.cancelBubble){
...
{
	eventObject = eventObject || getEventObject(eventObject);
	var target = eventObject.target || eventObject.sreElement;
	if(target.nodeType == ADS.nodeType.TEXT_NODE){
...
function getMouseButton(eventObject)
{
    ...
    // W3C Dom object have a toString method and in this case it should be MouseEvent
...
//get the key pressed from the object event
...
//change the style of a single element by id
...
{
	if(!(element = $(element))) return false;
	var classNames = getClassName(element);
