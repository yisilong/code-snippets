...
BOOL CMapiSession::Logon(const CString& sProfileName, const CString& sPassword, CWnd* pParentWnd, BOOL bForceDownload)
{
  ...
  ULONG nError = m_lpfnMAPILogon(nUIParam, nProfileLength ? CT2A(sProfileName) : NULL, nProfileLength ? CT2A(sPassword) : NULL, flags | MAPI_NEW_SESSION, 0, &m_hSession);
...
BOOL CMapiSession::Resolve(const CString& sName, lpMapiRecipDesc* lppRecip)
{
  ...
  ULONG nError = m_lpfnMAPIResolveName(m_hSession, 0, CT2A(sName), 0, 0, lppRecip);
  if (nError != SUCCESS_SUCCESS)
  {
...
BOOL CMapiSession::Send(CMapiMessage& message, BOOL bResolve, BOOL bInteractive, CWnd* pParentWnd, BOOL bSendAsHTML)
{
  ...
  CT2A szAsciiBody(message.m_sBody);  
	...
	if (bSendAsHTML) 
  {
    ...
    try
    {
      //HTML must be written out as ascii
		  ...
		  fileDefaulHtm.Write(szAsciiBody, static_cast<UINT>(strlen(szAsciiBody)));
		  fileDefaulHtm.Close();
    }
  ...
  CT2A szAsciiSubject(message.m_sSubject);
  mapiMessage.lpszSubject = szAsciiSubject;
  ...
  //This is the best we can do to simulate HTML email using just Simple MAPI
    ...
    mapiMessage.lpszNoteText = szAsciiBody;
...
BOOL CMapiSession::Find(CString& sMessageID, const CString& sSeedMessageID, FLAGS flFlags, const CString& sMessageType, BOOL bInteractive, CWnd* pParentWnd)
{
	//Validate our parameters
  ASSERT(MapiInstalled()); //MAPI must be installed
  ASSERT(m_lpfnMAPIFindNext); //Function pointer must be valid
  ASSERT(m_hSession); //MAPI session handle must be valid
  ...
  ULONG nError = m_lpfnMAPIFindNext(m_hSession, uiParam, sMessageType.GetLength() ? CT2A(sMessageType) : NULL, 
                                    sSeedMessageID.GetLength() ? CT2A(sSeedMessageID) : NULL, flFlags, 0, szAsciiMessageID);
	if (nError == SUCCESS_SUCCESS) 
  {
    bSuccess = TRUE;
    m_nLastError = nError;
...
BOOL CMapiSession::Read(const CString& sMessageID, CMapiMessage& message, FLAGS flFlags, BOOL bInteractive, CWnd* pParentWnd)
{
	//Validate our parameters
  ASSERT(MapiInstalled()); //MAPI must be installed
  ASSERT(m_lpfnMAPIReadMail); //Function pointer must be valid
  ASSERT(m_lpfnMAPIFreeBuffer); //Function pointer must be valid
  ASSERT(m_hSession); //MAPI session handle must be valid
  ...
  ULONG nError = m_lpfnMAPIReadMail(m_hSession, uiParam, CT2A(sMessageID), flFlags, 0, &mapiMessage);
	if (nError == SUCCESS_SUCCESS) 
  {
    bSuccess = TRUE;
	  m_nLastError = SUCCESS_SUCCESS;
    ...
    message.Initialise();
		message.m_From.m_sFriendlyName = CA2T(mapiMessage->lpOriginator->lpszName);
		message.m_From.m_sAddress = CA2T(mapiMessage->lpOriginator->lpszAddress);
		message.m_sSubject = CA2T(mapiMessage->lpszSubject);
		message.m_sBody = CA2T(mapiMessage->lpszNoteText);
		message.m_sDateReceived = CA2T(mapiMessage->lpszDateReceived);
    message.m_Flags = mapiMessage->flFlags;
    message.m_sMessageType = CA2T(mapiMessage->lpszMessageType);
...
BOOL CMapiSession::Delete(const CString& sMessageID, BOOL bInteractive, CWnd* pParentWnd)
{
	//Validate our parameters
	ASSERT(MapiInstalled()); //MAPI must be installed
	ASSERT(m_lpfnMAPIDeleteMail); //Function pointer must be valid
	ASSERT(m_hSession); //MAPI session handle must be valid
	ASSERT(sMessageID.GetLength() > 0); //Message ID should have been specified
	...
	ULONG nError = m_lpfnMAPIDeleteMail(m_hSession, uiParam, CT2A(sMessageID), 0, 0);
	if (nError == SUCCESS_SUCCESS) 
	{
		bSuccess = TRUE;
		m_nLastError = SUCCESS_SUCCESS;
...
{
  //What will the return value from this function
  BOOL bSuccess = FALSE;
