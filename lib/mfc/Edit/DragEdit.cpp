...
BOOL CDragEdit::m_bOleInit=FALSE;

BOOL CDragEdit::Init(int nFlags/* = DE_ENABLE_DRAG|DE_ENABLE_DROP*/)
{
    if (!m_bOleInit) 
	{
        if (!AfxOleInit()) 
...
CPoint CDragEdit::PosFromChar(UINT uChar)
{
    if (0==uChar)
        return CPoint(0,0);
    CPoint pt=CEdit::PosFromChar(uChar);
    if (pt.x<0 && pt.y<0) 
	{
        int nLine, nPos;
