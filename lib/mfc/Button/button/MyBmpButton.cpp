/////////////////////////////////////////////////////////////////////////////
/*
 *	MyBmpButton.cpp : implementation file
 *
 *					Author Ning Li
 *		 			     2004-5-28
 */
...
CMyBmpButton::CMyBmpButton()
{
	...
	m_bIsDefault		= FALSE;
	m_hCursor			= NULL;
	m_byTextAlign		= BMPBTN_ALIGN_CENTER;
...
BEGIN_MESSAGE_MAP(CMyBmpButton, CButton)
	//{{AFX_MSG_MAP(CMyBmpButton)
	ON_WM_MOUSEMOVE()
...
/////////////////////////////////////////////////////////////////////////////
// CMyBmpButton message handlers

void CMyBmpButton::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy bitmaps
...
void CMyBmpButton::PreSubclassWindow() 
{
	UINT nBS;
...
void CMyBmpButton::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
void CMyBmpButton::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	COLORREF crText = pDC->SetTextColor(m_crForceColors[BMPBTN_COLOR_FG_CLICK]);
	COLORREF crBack = pDC->SetBkColor(m_crForceColors[BMPBTN_COLOR_FG_CLICK]);
	...
	else
	{
		...
		if (m_bIsPressed)
		{
			pDC->SetTextColor(m_crForceColors[BMPBTN_COLOR_FG_CLICK]);
			pDC->SetBkColor(m_crForceColors[BMPBTN_COLOR_FG_CLICK]);
		} // if
		else 
		{
			//3D
			...
			if (m_bMouseOnButton) 
			{ 
				pDC->SetTextColor(m_crForceColors[BMPBTN_COLOR_FG_HIGH]); 
				pDC->SetBkColor(m_crForceColors[BMPBTN_COLOR_FG_HIGH]); 
			} // if
			else 
			{
				pDC->SetTextColor(m_crForceColors[BMPBTN_COLOR_FG_NORMAL]); 
				pDC->SetBkColor(m_crForceColors[BMPBTN_COLOR_FG_NORMAL]); 
			} // else
		} // else
...
void CMyBmpButton::SizeToContent(void)
{
	if (m_csBitmaps[0].hBitmap)
	{
...
BOOL CMyBmpButton::SetBitmaps(UINT nBitmapNormal, UINT nBitmapMouseDown, UINT nBitmapHigh, UINT nBitmapDisable)
{
...
BOOL  CMyBmpButton::SetBitmaps(HBITMAP hBitmapNormal, HBITMAP hBitmapMouseDown, HBITMAP hBitmapHigh, HBITMAP hBitmapDisable)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
...
void CMyBmpButton::CancelHover(void)
{
	if (m_bMouseOnButton)
	{
		m_bMouseOnButton = FALSE;
...
#ifdef	BMPBTN_USE_SOUND
BOOL CMyBmpButton::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
...
BOOL CMyBmpButton::SetForceColor(COLORREF crColorNormal, COLORREF crColorMouseDown, COLORREF crColorHigh, BOOL bRepaint)
{
	// Set new color
	m_crForceColors[BMPBTN_COLOR_FG_NORMAL] = crColorNormal;
	m_crForceColors[BMPBTN_COLOR_FG_CLICK] = crColorMouseDown;
	m_crForceColors[BMPBTN_COLOR_FG_HIGH] = crColorHigh;
...
BOOL CMyBmpButton::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	if (m_hCursor != NULL)
	{
		::SetCursor(m_hCursor);
...
void CMyBmpButton::OnKillFocus(CWnd* pNewWnd) 
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
}

BOOL CMyBmpButton::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
...
BOOL CMyBmpButton::SetDrawFocusRect(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
BOOL CMyBmpButton::SetFocusRectDeflateRect(int x, int y)
{
	m_iDeflateRect_x = x;
	m_iDeflateRect_y = y;
...
void CMyBmpButton::ReconstructFont(void)
{
	m_font.DeleteObject();
...
CMyBmpButton& CMyBmpButton::SetFontBold(BOOL bBold, BOOL bRepaint)
{	
...
CMyBmpButton& CMyBmpButton::SetFontName(const CString& strFont, BYTE byCharSet /* Default = ANSI_CHARSET */, BOOL bRepaint)
{	
...
CMyBmpButton& CMyBmpButton::SetDrawText(BOOL bDraw, BOOL bRepaint)
{
	m_bDrawText = bDraw;
...
CMyBmpButton& CMyBmpButton::SetFontUnderline(BOOL bSet, BOOL bRepaint)
{
...
CMyBmpButton& CMyBmpButton::SetFontItalic(BOOL bSet, BOOL bRepaint)
{
	m_lf.lfItalic = bSet;
	ReconstructFont();
...
CMyBmpButton& CMyBmpButton::SetFontSize(int nSize, BOOL bRepaint)
{
	CFont cf;
	LOGFONT lf;
...
CMyBmpButton& CMyBmpButton::SetFont(LOGFONT lf, BOOL bRepaint)
{
...
CMyBmpButton& CMyBmpButton::SetFont3D(BOOL bSet, int i3dx, int i3dy, BOOL bRepaint)
{
	m_bFont3d = bSet;
	m_iText3d_x = i3dx;
	m_iText3d_y = i3dy;
...
CMyBmpButton& CMyBmpButton::SetText3DBKColor(COLORREF cr3DBKColor, BOOL bRepaint)
{
...
BOOL CMyBmpButton::SetTextAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case BMPBTN_ALIGN_LEFT:
...
CMyBmpButton&  CMyBmpButton::SetDrawTextAsDisable(BOOL bSet, BOOL bRepaint)
{
